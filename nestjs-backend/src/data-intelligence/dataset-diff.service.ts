...
export class DatasetDiffService {
  ...
  async generateDiffs(newDatasetId: number): Promise<void> {
    ...
    if (!previousDataset) {
      this.logger.log(
        `No previous dataset found. Base initialization completed for ${newDatasetId}.`,
      );
      return;
    }

    this.logger.log(
      `Comparing Dataset ${newDatasetId} against previous Dataset ${previousDataset.id}...`,
    );

    const newRecords = await this.em.find(LiveMatrixData, {
      dataset: newDataset,
    });
    const oldRecords = await this.em.find(LiveMatrixData, {
      dataset: previousDataset,
    });

    const newMap = new Map(
      newRecords.map((r) => [`${r.sector}-${r.jobRole}`, r]),
    );
    const oldMap = new Map(
      oldRecords.map((r) => [`${r.sector}-${r.jobRole}`, r]),
    );
    ...
    if (diffs.length > 0) {
      this.logger.log(
        `Generated ${diffs.length} diffs between Dataset ${newDatasetId} and ${previousDataset.id}.`,
      );
      this.em.persist(diffs);
      await this.em.flush();
    } else {
      this.logger.log(
        `No variations detected between Dataset ${newDatasetId} and ${previousDataset.id}.`,
      );
    }
  }
}
