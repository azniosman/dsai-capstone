    ...
    try {
      ...
      if (res.data) {
        setData({
          ...
          years_experience: res.data.years_experience ?? res.data.yearsExperience ?? 0,
          age: res.data.age,
          skills: res.data.skills || [],
          resume_text: res.data.resume_text ?? res.data.resumeText ?? "",
          is_career_switcher: res.data.is_career_switcher ?? res.data.isCareerSwitcher ?? false,
        });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (data.id) {
        ...
        const { id: _id, ...patchBody } = data;
        await api.patch(`/api/profile/${data.id}`, patchBody);
